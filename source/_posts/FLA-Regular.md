...
若有$\epsilon-NFA$ E, 构造$DFA$ D, 使$\mathbf{L}(D) = \mathbf{L}(E)$, 方法与子集构造法类似, 但使用$\epsilon$闭包代替状态转移后的集合。

设$\epsilon-NFA\ E=(Q_E,\Sigma,\delta_E,q_E,F_E)$，构造$DFA\ D=(Q_D,\Sigma,\delta_D,q_D,F_D)$，规则如下：

1. $Q_D=2^{Q_E}=\{S|S \subset Q_E\}$，但实际上只有满足$S=ECLOSE(S)是有效的;
2. $Q_D=ECLOSE(q_E)$，即使用$q_E$的闭包作为D的开始状态
3. $F_D=\{S|S \in Q_D,S\cap F_E \ne \emptyset\}$，是要包含$F_E$，就是$D$的接受状态
4. $\delta_D:\forall S \in Q_D,\delta(S,a)=ECLOSE(\bigcup_{p \in S}\delta_E(p,a))$-闭包里面的状态接受$a$字符之后转移到的状态的闭包的并。

$\epsilon-NFA$与$DFA$等价性
--------------------------
...
3. $\Sigma$中的任意字符a，都是一个正则表达式，分别表示语言$\{a\}$
4. 如果正则表达式$r$和$s$分别比啊是语言$R$和$S$，则$r+s,rs,r^*$和$(r)$也是正则表达式，分别表示语言$R\cup S,RS,R^*$和R

此外$r^+=rr^*$也称为*正闭包*，显然$r^*=r^++\epsilon$，而且当且仅当$\epsilon \in \mathbf{L}(r)$。
...
- $\emptyset + L = L + \emptyset$ 并运算的单位元$\emptyset$
- $\epsilon L = L \epsilon = L$ 连接运算的单位元$\epsilon$
- $\emptyset L = L\emptyset = L$ 连接运算的零元
...
$DFA, NFA, \epsilon-NFA$和正则表达式在表示语言的能力上是等价的.
...
DFA$\implies$正则表达式，递归构造$R_{i,j}^{(k)}$
---------------------------------------------

DFA$\implies$正则表达式，状态消除
------------------------------

正则表达式$\implies$DFA，构造$\epsilon-NFA$
-----------------------------------------
